...
Windows 版通知带有 **Open News** 按钮。请使用这个按钮打开生成的 News；未签名 Win32 程序的通知正文点击在 Windows 上不稳定。
...
| 源码运行 | 开发者或 Intel Mac 用户 | 克隆仓库后用 Python 虚拟环境运行 |

> 当前 CI 自动构建的是 `macOS arm64` DMG。Intel Mac 用户可以源码运行，或在 Intel Mac 上执行 `bash build_dmg.sh` 自行构建。
...
打开 DMG 后，把 `Zotero 简报.app` 拖到“应用程序”。由于当前构建未签名和未 notarize，首次启动如果被 Gatekeeper 拦截，可以右键打开，或在系统设置的安全性页面允许打开。
...
GitHub Actions 会在 Windows 和 macOS runner 上运行测试。
...
macOS DMG job 会在 GitHub-hosted macOS runner 上执行 `hdiutil verify`，挂载 DMG，并检查 `Zotero 简报.app` 和 launcher 是否存在。GitHub-hosted runner 适合验证构建和可挂载性，但不能可靠模拟用户手动点击通知、授权弹窗或菜单栏交互。
...
**Windows 能收到通知，但点通知正文没反应**

使用通知里的 **Open News** 按钮。当前未签名 Win32 构建不能保证通知正文点击能触发应用。
