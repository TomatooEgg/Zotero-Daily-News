...
def _terminal_notifier_execute_cmd(note_id: str, hub_path: Path | None = None) -> str:
    ...
    if getattr(sys, "frozen", False):
        return (
            ...
            f"{shlex.quote(sys.executable)} --open-target {shlex.quote(note_id)}{hub_arg}"
        )
    py = _project_python()
    inner = (
        f"cd {shlex.quote(str(SCRIPT_DIR))} && "
    ...
    try:
        ...
        if sp.run(["sysctl", "-n", "hw.optional.arm64"], capture_output=True, text=True).stdout.strip() == "1":
            return (
                f"cd {shlex.quote(str(SCRIPT_DIR))} && NO_PROXY=127.0.0.1,localhost,::1 "
                f"arch -arm64 {shlex.quote(py)} -m zotero_daily_news.notifier "
                f"--open-target {shlex.quote(note_id)}{hub_arg}"
            )
    except OSError:
        pass
